...
function checkForm () {
  ...
  else if ((document.form2.czas_umowy[1].checked && document.form2.wybor1[0].checked) || (document.form2.czas_umowy[2].checked && document.form2.wybor1[0].checked) || document.form2.wybor1[3].checked){
...
{
if (document.form2.czas_umowy[2].checked)
{
document.form2.wybor1[0].checked=true;
}
  
if (((document.form2.czas_umowy[1].checked) && (document.form2.wybor1[0].checked)) || ((document.form2.czas_umowy[2].checked) && (document.form2.wybor1[0].checked)) || (document.form2.wybor1[3].checked))
{
...
?>
	...
	<INPUT TYPE="radio" NAME="czas_umowy" VALUE="do10" onClick="Show_Stuff(display2)">Umowa podpisana do 10 dni<BR>
...
if (($czas_umowy=='do10') or ($czas_umowy=='do10brak')) $umowa_do10dni='tak';
	else $umowa_do10dni='';
...
if (($wybor1=='zrezygnowal') and ($czas_umowy=='do10'))
	{
		$name=corpol($name);
		$lastname=corpol($lastname);
		$city=corpol($city);
		$street=corpol($street);
		$query1 = $db->query("INSERT INTO callregistry_anty_10 (oper_id, data, czas, cust_id, name, lastname, ul, street, house, flat, zip, city, tel, signdate, pan, socjal) VALUES ('$xmbid','$data','$czas', '$cust_id', '$name', '$lastname', '$ul', '$street', '$house', '$flat', '$zip', '$city', '$tel', '$signdate', '$pan', '$socjal')");
...
elseif ($wybor1=='nie_zrezygnowal_anul')
	{
		...
		$city=corpol($city);
		$street=corpol($street);
		$query1 = $db->query("INSERT INTO callregistry_anty_anul (oper_id, data, czas, cust_id, name, street, zip, city, tel, signdate) VALUES ('$xmbid','$data','$czas', '$cust_id', '$name', '$street', '$zip', '$city', '$tel', '$signdate')");
	}
