...
function fix_data($fdata) // zamienia date 2006-2-7 na 2006-02-07
{
$arfields=explode("-",$fdata);

$ret_data=$arfields[0];

foreach ($arfields as $ttemp)
	{
	if (strlen($ttemp)==1) $ret_data=$ret_data."-0".$ttemp;
	elseif (strlen($ttemp)==2) $ret_data=$ret_data."-".$ttemp;
	}

return $ret_data;
}

if ($new_datapocz) $new_datapocz=fix_data($new_datapocz);
if ($new_datakon) $new_datakon=fix_data($new_datakon);
...
//edytowanie w tabeli
		...
		if ($x>0) echo "<font color=red>Odblokowano $x rekordow zablokowanych na Twoim loginie</font><BR>";
				...
				echo "
					...
					<TR>
						...
						<TD>
							<script language=javascript>
								var basicCal1 = new calendar(\"FIELD:document.frmWysz.new_datapocz;FORMAT:2;DELIMITER:-;\");
								basicCal1.writeCalendar();
							</script>
						</TD>
					</TR>
					...
					<TR>
						...
						<TD>
							<script language=javascript>
								var basicCal2 = new calendar(\"FIELD:document.frmWysz.new_datakon;FORMAT:2;DELIMITER:-;\");
					...
					<TR>
						...
						<TD>
							...
							<OPTION value=\"\"> brak statusu BE</OPTION>
							<OPTION value=\"za\"> Klient zablokowany</OPTION>
							<OPTION value=\"up\"> caly T2_upsell_1</OPTION>
							<OPTION value=\"ob\"> bonus do przyznania</OPTION>
							</SELECT>
						</TD>
						<TD>
	...
	elseif ($action=='stat')
	{
				...
				";
						...
						elseif($status_typ=='ob')
							{
								$status_filtr="status_be='ob'";

							}
						elseif($status_typ=='up')
							{
								$status_filtr="source='T2_upsell_1' AND (ulica IS NULL OR ulica ='')";
								$comstat='upselling - ';
				...
				if ($leadcount>0)
				{
					...
					while ($id[$i])
					{
						if (($status[$i]) AND ($edytuj[$i]))
						{
							...
							if( $temp_status=='ok' || $temp_status=='ob' ) {
								include("be_sales_20090301.php");
							}
						...
						elseif ($status[$i])
						{
							...
							$query = $db->query("INSERT INTO callregistry_promo_log (lead_id, status_time, status, status_oper_id) VALUES ($temp_id, '$data', '$temp_status', $xmbid)");
							$query2 = $db->query("UPDATE callregistry_promo SET in_use = NULL, status_oper_id = $xmbid, status_be = '$temp_status', status_date = '$data' WHERE id=$temp_id AND (status_be IS NULL OR status_be = '')");
							$query3 = $db->query("UPDATE callregistry_promo SET in_use = NULL, status_be = '$temp_status' WHERE id=$temp_id AND status_be = 'ob'");
							if( $temp_status=='ok' || $temp_status=='ob' ) {
								include("be_sales_20090301.php");
							}							
